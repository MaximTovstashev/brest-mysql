...
var Table = function(db, settings) {
        ...
        if (err) {
            ...
            self.queries = _.defaults(self.queries || {}, {
                ...
                util.format("SELECT {{columns}}{{select}} " +
                    "FROM `%s`{{join}} " +
                    "WHERE {{whereClause}}{{where}}{{group}}{{order}} LIMIT 1", self.name),

                list: self.settings.queries.list ||
                util.format("SELECT {{columns}}{{select}} " +
                    "FROM `%s`{{join}} " +
                    "WHERE 1{{where}}{{group}}{{order}}{{limit}}", self.name),
    ...
    this.resumePersistentUpdates = function(callback, preventUpdating){
        if (_.isFunction(callback)) {
            self.persistentCallbacksStack.push(callback);
        }
        ...
        if (self.persistentUpdatesSuspended == 0 && !preventUpdating) {
            self.updatePersistent(function(err){
                ...
                if (self.persistentCallbacksStack.length) {
                    _.each(self.persistentCallbacksStack, function(delayedCallback){
                        delayedCallback(err);
                    })
                }
            })
    ...
    this.updatePersistent = function(callback){
        if (self.persistent && self.persistentUpdatesSuspended == 0) {
    ...
    this.insert = function(data, options, callback) {
        ...
        _.each(data, function(value, column){
            ...
            if (_.isObject(value)){
                if (self.columns[column].isDynamic) data[column] = self.flattenJSON(value, 'COLUMN_CREATE');
                else data[column] = JSON.stringify(value);
            }
        });
        ...
        var q = self.db.query(sql, data, function(err, res){
            if (err) callback(err);
            else {
