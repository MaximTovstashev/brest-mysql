...
var Table = function(db, settings) {
        ...
        if (err) {
            ...
            _.each(columns, function(column){
                self.columns[column.Field] = {
                    ...
                    isDynamic: (self.dynamic.indexOf(column.Field) > -1)
                };
                if (column.Key == 'PRI') {
            ...
            self.queries = _.defaults(self.queries || {}, {
                ...
                util.format("SELECT {{columns}}{{select}} " +
                    "FROM `%s`{{join}} " +
                    "WHERE {{whereClause}}{{where}} LIMIT 1", self.name),
                list: self.settings.queries.list ||
                util.format("SELECT {{columns}}{{select}} " +
                    "FROM `%s`{{join}} " +
                    "WHERE 1{{where}}{{order}}{{limit}}", self.name),
            ...
            self.updatePersistent(function(err){
                if (err) console.log('ERROR: persistent fields creation failed for ', self.name, 'with error:\n',err);
            });
        }
    });
    ...
    this.suspendPersistentUpdates = function(){
        self.persistentUpdatesSuspended = true;
    };

    /**
     * Make persistent updates possible again
     */
    this.resumePersistentUpdates = function(callback){
        self.persistentUpdatesSuspended = false;
        self.updatePersistent(function(err){
            if (err) console.log('ERROR: persistent fields update failed on resume for ', self.name, 'with error:\n', err);
            callback(err);
        });
    };
    ...
    this.updatePersistent = function(callback){
        if (self.persistent && !self.persistentUpdatesSuspended) {
            async.each(_.keys(self.persistent), function(key, callback){
                self.persistent[key](function(err, data){
                    if (err) {
                        self.p[key] = false;
                        callback(err);
                    }
    ...
    this.row = function(ids, filters, callback){
        ...
        _.each(self.columns, function(column, name){
            if (column.isDynamic) {
                columns.push(util.format('COLUMN_JSON(`%s`.`%s`) as `%s`', self.name, name, name));
            } else {
    ...
    this.insert = function(data, options, callback) {
        ...
        var sql = self.prepareQuery(self.queries.insert, data);

        var duplicate = "";
        if (options.duplicate){
            _.each(_.keys(data), function(key){
                var replaceVal = key;
                if (_.isObject(replaceVal)) replaceVal = JSON.stringify(replaceVal);
                options.duplicate = options.duplicate.replace(":"+key, self.esc(data[key]));
            });
            duplicate = " ON DUPLICATE KEY UPDATE " + options.duplicate;
        }
        ...
        var q = self.db.query(sql, _.values(data), function(err, res){
            if (err) callback(err);
            else {
    ...
    this.update = function(data, options, callback) {
        ...
        var sql = self.prepareQuery(self.queries.update, data);
        ...
        _.each(columns, function(i, column){
            columnValues.push(util.format('`%s` = ?', column));
        });
        var valuesStr = columnValues.join(', ');

        sql = sql.replace('{{columns}}', valuesStr);
    ...
    this.del = function(ids, callback) {
        ...
        return self.db.query(self.queries.del, ids, function(err){
            if (err) callback(err);
            else {
    ...
    this.queryRow = function(sql, params, callback) {
        sql = self.prepareQuery(sql, params);
    ...
    this.prepareQuery = function(query, data) {
        _.each(_.keys(data), function(key){
            var replaceVal = key;
            if (_.isObject(replaceVal)) replaceVal = JSON.stringify(replaceVal);
            query = query.replace(":"+key, self.esc(data[key]));
        });
        return query;
    };
